...
class SetCurrentPlayerActionTest {
    ...
    @Test
    void jumpsTurnDirectlyToTargetPlayer() {
        GameState state = new GameState(List.of(ALICE, BOB, CAROL));
        SetCurrentPlayerAction action = new SetCurrentPlayerAction();
        RuleContext context = new RuleContext(state, Event.of("PLAY_CARD")).withTarget(CAROL);
